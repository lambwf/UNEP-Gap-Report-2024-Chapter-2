...
p1 <- data_luc_diff %>% ggplot(.,aes(x=year,y=value,colour=var)) +
  ...
  labs(title="Differences in net land use change (LULUCF) estimates",
       ...
       caption="⌂ UNEP Emissions Gap Report 2024    ⌂ Data: GCB 2023, Grassi et al. 2024  \n⌂ Note: the bookkeeping average and inventory estimates differ due to alternative system boundary definitions")
...
colours_top6 <- c(colours_top6,"Rest of world" = "#818181ff")
...
p2 <- labels %>% 
  ggplot(.,aes(x=1,y=label_position,label=label,color=group)) +
  geom_text_repel(
            hjust=0,
            direction = "y",
            point.size = NA,
