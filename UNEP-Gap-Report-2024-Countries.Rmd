...
```{r figure_changes_by_country,fig.width=7,fig.height=4,fig.path="results/",dev=c('png','pdf')}
...
data_changes_eu <- left_join(data_changes,cc_EU,by="iso") %>%
  filter(!is.na(EU)) %>% 
  mutate(iso="EU27") %>% 
  select(-EU) %>%
...
data_changes_eu <- rbind(data_changes_eu,data_ghg %>% 
                       filter(iso=="EUR") %>% 
                       filter(sector_lv2=="AFOLU: LULUCF") %>% 
                       mutate(iso="EU27") %>% 
                       mutate(country="European Union"))

data_changes <- rbind(data_changes %>% filter(iso!="EUR"),data_changes_eu)
data_changes <- left_join(data_changes,cc_EU,by="iso") %>%
  filter(is.na(EU))
...
countries <- unique(data_changes$country)
...
blarg <- data_changes %>% 
  filter(year==2023) %>% 
  mutate(biggest_changes=abs(change_abs)) %>% 
  arrange(desc(biggest_changes)) %>% 
  head(7) %>% 
  mutate(include=1) %>% 
  select(country,include)

data_changes <- left_join(data_changes,blarg)
data_changes <- data_changes %>% 
  mutate(country=ifelse(is.na(include),"Rest",country)) %>% 
  group_by(country,year) %>% 
  summarise(value=sum(value),change_abs=sum(change_abs))
...
plot_country <- function(country_iso,data_ghg,data_crfs,data_indicators,pull_out_year) {
  ...
  p1 <- ggplot() +
    geom_area(data=data_countries,color="#636363",aes(x=year,y=value,fill=sector_lv2)) +
  ...
  p <-  wrap_elements(p1 + p2 + plot_layout(widths=c(7,1))) + plot_spacer() +
    wrap_elements(plot_spacer() / p3 / plot_spacer() + plot_layout(heights=c(0.5,10,ifelse(is_crf=="T",1.75,1))) & theme(plot.background=element_blank())) + 
...
# 
#   ggsave(plot,filename = paste0(countries$country[i],".png"),path = "results/countries/",device = "png",height = 4,width=9,dpi=300)
#   ggsave(plot,filename = paste0(countries$country[i],".png"),path = "results/countries/lowres/",device = "png",height = 4,width=9,dpi=150)
#   ggsave(plot,filename = paste0(countries$country[i],".pdf"),path = "results/countries/",device = "pdf",height = 4,width=9)
#   ggsave(plot,filename = paste0(countries$country[i],".svg"),path = "results/countries/",device = "svg",height = 4,width=9)
# 
# 
# }
...
# ggsave(p,filename = "African Union.png",path = "results/countries/lowres/",device = "png",height = 4,width=9,dpi=150)
# ggsave(p,filename = "African Union.pdf",path = "results/countries",device = "pdf",height = 4,width=9)
# ggsave(p,filename = "African Union.svg",path = "results/countries",device = "svg",height = 4,width=9)
