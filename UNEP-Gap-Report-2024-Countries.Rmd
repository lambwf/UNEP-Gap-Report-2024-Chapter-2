...
```{r plot_countries,fig.height=4,fig.width=8}
country_iso="BRA"
annotations="yes"
...
```{r plots, fig.width=8.5,fig.height=4}
...
```{r country_test, fig.width=8.5,fig.height=4}
...
```{r figure_top_emitters, fig.width=8,fig.height=12, fig.path="results/",dev=c('png','pdf','svg')}
...
p1 <- plot_country("CHN",data_ghg,data_indicators,"no")
p2 <- plot_country("USA",data_ghg,data_indicators,"no")
p3 <- plot_country("IND",data_ghg,data_indicators,"no")
p4 <- plot_country("EU27",data_ghg,data_indicators,"no")
p5 <- plot_country("RUS",data_ghg,data_indicators,"no")
p6 <- plot_country("BRA",data_ghg,data_indicators,"no")

ggsave(wrap_elements(p1) / wrap_elements(p2) / wrap_elements(p3),filename = "figure_top_emitters_1.png",path = "results/",device = "png",height = 12,width=8,dpi=300)
ggsave(wrap_elements(p4) / wrap_elements(p5) / wrap_elements(p6),filename = "figure_top_emitters_1.png",path = "results/",device = "png",height = 12,width=8,dpi=300)
...
data_output <- data_output %>% 
  group_by(iso) %>% 
  mutate(`Total GHG emissions (MtCO2e)`=sum(value)) %>% 
