...
data_countries_eu <- left_join(data_countries,cc_EU,by="iso") %>% 
  filter(dataset!="EDGARv9") %>% 
  filter(dataset!="CRFs_2023") %>% 
  filter(EU==1) %>% 
  mutate(iso="EU27") %>% 
  mutate(country="European Union") %>% 
  group_by(across(c(-value))) %>% 
...
data_countries <- left_join(data_countries,cc_G20,by="iso")
data_countries <- data_countries %>% 
  filter(G20==1) %>% 
  select(-G20)

data_countries <- data_countries %>% 
  arrange(iso,country,year,gas) %>% 
...
# data_countries <- spread(data_countries,dataset,value)
# 
# 
# wb <- createWorkbook()
# 
# addWorksheet(wb,"g20_totals_comparison")
# writeData(wb, sheet = "g20_totals_comparison", data_countries, colNames = T, rowNames = F)
# 
...
```{r plot_high_emitters,fig.height=8,fig.width=10,fig.path="results/",dev=c('png','pdf')}
...
data_table_countries <- data_table_countries %>% 
  filter(year>=1990) %>% 
  mutate(value=ifelse(year==2022 & sector_lv2=="AFOLU: LULUCF",NA,value)) %>% 
...
data_indicators_au <- gather(data_indicators_au,var,value,-iso,-country) %>%
  ...
  mutate(iso="AU55")
data_indicators <- rbind(data_indicators,spread(data_indicators_au,var,value))
...
data_indicators_g20 <- left_join(data_indicators,cc_G20 %>% add_row(iso="AU55",G20="1"),by="iso") %>%
  filter(!is.na(G20)) %>%
  mutate(iso="G20") %>%
  select(-G20) %>% 
...
data_indicators_g20 <- gather(data_indicators_g20,var,value,-iso,-country) %>%
  ...
  summarise(value=sum(value,na.rm=TRUE)) %>%
  mutate(country="G20")
data_indicators <- rbind(data_indicators,spread(data_indicators_g20,var,value))
...
data_indicators <- data_indicators %>% 
  ...
  mutate(ghg_total=round(ghg_total_2023,0)) %>% 
  mutate(ghg_total=paste0(ghg_total," (",ghg_fraction,")"))
...
data_indicators <- data_indicators %>% 
  ...
  mutate(co2_historic = paste0(round(co2_historic,0)," (",co2_historic_fraction,")"))

data_indicators <- data_indicators %>% 
  select(iso,country,ghg_total,change_rel,ghg_per_capita,co2_historic) %>% 
  filter(iso %in% c("CHN","USA","IND","EU27","RUS","BRA","AU55","LDC","G20"))

data_indicators$iso <- as.factor(data_indicators$iso)
data_indicators$iso <- fct_relevel(data_indicators$iso,"CHN","USA","IND","EU27","RUS","BRA","AU55","LDC","G20")
data_indicators <- data_indicators %>% 
  arrange(iso)
...
data_indicators <- data_indicators %>%
  set_variable_labels(ghg_total="MtCO2e (% of total)",
                      change_rel="%",
                      ghg_per_capita="tCO2e/person",
                      co2_historic="GtCO2 (% of total)")


data_indicators <- data_indicators %>%
  rename(`Total GHG emissions in 2023`=ghg_total) %>% 
  rename(`Change in total GHG emissions, 2022-2023`=change_rel) %>% 
  rename(`Per capita GHG emissions in 2022`=ghg_per_capita) %>% 
  rename(`Historic CO2 emissions, 1850-2023`=co2_historic)



addWorksheet(wb,"table_2")
writeData(wb, sheet = "table_2", data_indicators, colNames = T, rowNames = F)
...
```{r plot_countries,fig.height=4,fig.width=9}
country_iso="AU"

plot_country <- function(country_iso,data_ghg,data_crfs) {
  ...
  p1 <- data_countries %>% 
    ...
    labs(title=data_countries$country,
         ...
         caption="Sources: EDGAR v9, Grassi et al. 2021, UNFCCC")
...
countries <- data_ghg %>% ungroup() %>% select(iso,country) %>% distinct()
countries <- countries %>% filter(country %in% c("Hungary","Bulgaria","Poland","Romania","Slovenia"))

for (i in 1:length(countries$iso)) {

  ggsave(plot_country(countries$iso[i],data_ghg,data_crfs),filename = paste0(countries$country[i],".png"),path = "results/countries",device = "png",height = 4,width=9,dpi=300)
  ggsave(plot_country(countries$iso[i],data_ghg,data_crfs),filename = paste0(countries$country[i],".pdf"),path = "results/countries",device = "pdf",height = 4,width=9)
...
#   mutate(country="African Union")
# 
# plot_country("AU",blarg,data_crfs)
# 
...
plot_country("DEU",data_ghg,data_crfs)
plot_country("AIA",data_ghg,data_crfs)
plot_country("ABW",data_ghg,data_crfs)
