...
gwps <- gwps %>% 
  select(gas=Species,gwp100=AR6GWP100)
...
## Add cement carbonation https://globalcarbonbudget.org/carbonbudget2023/
...
data_gfed_ch4 <- read.xlsx("sources/GFED4.1s_2024.xlsx",sheet="GFED_CH4",rows = c(14,105,122))    ## as in GCB, only use global tropical & SE Asia peat fires
...
data_gfed_n2o <- read.xlsx("sources/GFED4.1s_2024.xlsx",sheet="GFED_N2O",rows = c(14,105,122))    ## as in GCB, only use global tropical & SE Asia peat fires
...
plot_ghgs %>% ggplot(aes(x=year,y=value,fill=fct_rev(gas))) +
  ...
  geom_text(inherit.aes = FALSE, data=plot_ghgs_totals %>% filter(year %in% c(1990,2000,2010,2020,2023)),aes(x=year,y=63,label=paste(signif(totals,3))),
